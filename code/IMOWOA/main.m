...
while tt < Max_iter
    ...
    TT = floor(Max_iter/50);
    if (1<= tt) && (tt<= TT)
        a = (((tt-1)/(TT-1))^2) + 1;
    else
        a = 0.5*(((tt-TT-1)/(Max_iter-TT-1))^2)+0.5;
    end
    % a = 2-tt*((2)/Max_iter); 
    a2 = -1+tt*((-1)/Max_iter);
    F1 = pop(F{1});
    ...
    for i = 1:size(new_pos,1)
        ...
        for j = 1:size(new_pos,2)
            ...
            elseif p>=0.5
                ...
                new_pos(i,j) = distance2Leader*exp(b.*l).*cos(l.*2*pi)+Leader_pos(j) + sin(new_pos(i,j));
            end
            X_new_pos(i,j) = new_pos(i,j) + normrnd(0,1)* new_pos(i,j);
        end
        X_angents_pos(i).x_kt = (reshape(X_new_pos(i,:),11,30))';
        angents_pos(i).x_kt = (reshape(new_pos(i,:),11,30))';
        angents_pos(i).x_kt = cheak(abs(angents_pos(i).x_kt));
        X_angents_pos(i).x_kt = cheak(abs(X_angents_pos(i).x_kt));
        new_obj = Objective(angents_pos(i),1);
        X_new_obj = Objective(X_angents_pos(i),1);
        new_obj.obj = [new_obj.obj1;new_obj.obj2;new_obj.obj3];
        X_new_obj.obj = [X_new_obj.obj1;X_new_obj.obj2;X_new_obj.obj3];
        if dominate(X_new_obj, new_obj)
            popc(i).obj = X_new_obj.obj;
            transposition_pos = (X_angents_pos(i).x_kt)';
            pos(i,:) =  transposition_pos(:);
            popc(i).value = pos(i,:);
        else
            popc(i).obj = new_obj.obj;
            transposition_pos = (angents_pos(i).x_kt)';
            pos(i,:) =  transposition_pos(:);
            popc(i).value = pos(i,:);
        end
        for t = 1:1:T
